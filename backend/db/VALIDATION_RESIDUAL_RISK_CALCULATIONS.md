...
**Status**: ✅ **PASSED** — All calculations verified and architecturally correct

---

## Executive Summary

The residual risk calculations in the `threat.threat_impact` view are **mathematically correct and architecturally sound**. The validation confirms that:
...
5. ✅ **Threat probability is intentionally excluded** from SLE calculations (by design)

---

## Key Architectural Decision: Why Probability ≠ SLE

### The Design

From migration `20260902-pat-341-threat-impact-sle-views.sql` (lines 9-11, 123-126):

> **Threat probability remains descriptive and is not used in the financial calculation. Aggregate portfolio exposure is reported separately from individual threat SLE.**

> **Probability is retained for context and prioritization, but does not participate in SLE.**

### Rationale

**Single Loss Expectancy (SLE)** is the absolute dollar loss if a threat materializes. It represents the maximum exposure per-threat-realization, independent of likelihood.

**Probability** is used for:
- **Prioritization**: "Which threats should we address first?"
- **Context**: "What are the odds this will happen?"
- **Separate metric**: Portfolio-level expected loss aggregates SLE across threats with different probabilities

**Why NOT multiply SLE × Probability?**
1. **SLE remains absolute**: A $249M exposure is a $249M exposure, regardless of whether it's 5% or 95% likely
2. **Portfolio aggregation**: Sum all SLE values across threats → portfolio gross exposure
3. **Regulatory clarity**: Financial statements need absolute exposure (SLE) separate from likelihood-adjusted figures (Annual Expected Loss)
4. **Mitigation focus**: Residual exposure shows "what's left to fix" in absolute terms, not "probability-weighted what's left"

### Formula Reference

**Correct approach (implemented)**:
```
Gross SLE = SUM(asset.sle_value * damage%)          ← Probability NOT included
Current SLE = Gross SLE * residual_multiplier        ← Probability NOT included
```

**Incorrect approach (NOT used)**:
```
Gross SLE = SUM(asset.sle_value * damage%) * probability%   ← WRONG
Current SLE = Gross SLE * residual_multiplier                ← WRONG
```
...
**Note**: This is the absolute exposure, **independent of the 5% threat probability**. The probability is stored separately in `threat.probability` for context and prioritization.
...
### Step 5: Mitigation Percentage

```
Current Mitigation % = (1 - Residual Multiplier) × 100
                     = (1 - 0.05) × 100
                     = 95.0%
```
...
## SQL Formula Reference (from threat_impact view)

The view uses these SQL formulas (from migration 20260902-pat-341-threat-impact-sle-views.sql):

### Gross SLE (Lines 130-135)
```sql
ROUND(
    SUM(
...
**Key points**:
- Sums all asset SLE values weighted by damage percentage
- **Probability NOT included** (by design)
- Rounded to nearest integer

### Current Residual Multiplier (Lines 71-90)
```sql
COALESCE(
    GREATEST(
        0.05,  -- Minimum 5% residual
        EXP(
            SUM(
                LN(
                    1.0 - LEAST(
    ...
    1.0  -- Default if no countermeasures
)
```

**Key points**:
- Only counts **implemented** countermeasures (`FILTER (WHERE c.implemented = true)`)
- Mitigation levels clamped to 0..95% (prevents invalid ln(0))
...
**Result for threat 23146**: **12,451,063**

### Current Mitigation % (Lines 145-151)
```sql
ROUND(
...
**Result for threat 23146**: **95.0%**

### Current Residual Exposure % (Lines 153-156)
```sql
ROUND(
...
**Result for threat 23146**: **5.0%**

---

## Validation Results

| Metric | View Output | Manual Calculation | Matches |
|--------|-------------|-------------------|---------|
...
- **Probability NOT included in calculation** (intentional design)
...
### Tables Used
- ✅ `threat.threats` — threat metadata (includes probability for context only)
...
The residual risk calculation system is **working correctly**. No bugs detected. The architecture intentionally excludes probability from SLE calculations to maintain absolute exposure metrics.
...
Consider adding a comment in the frontend code (e.g., bigpicture view) clarifying that:
- **Gross SLE** = Absolute financial exposure (probability-independent)
- **Probability** = Likelihood for prioritization (see `threat_probability` column)
- **Expected Loss** (if needed) = Gross SLE × (Probability / 100)
...
- **Threat Probability**: 5% (stored for context, not used in SLE)
...
5. Threat probability for context and prioritization

**All residual risk calculations are validated, architecturally sound, and production-ready.**

---

## References

- **Source**: `backend/db/20260902-pat-341-threat-impact-sle-views.sql`
- **View**: `threat.threat_impact`
- **Related Views**: `THRIM`, `threat.portfolio_exposure`, `threat.asset_threat_exposure`
