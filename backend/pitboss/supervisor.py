...
class PitbossSupervisor:
    ...
    async def process_envelope(self, envelope_dict: dict):
        ...
        if not verb_str:
            error_msg = "Missing or empty 'verb' field. Must be: RULE, CONTENT, CARD, GENERATE, ENRICH, FEELGOOD, or PROFILE"
        ...
        if verb_str not in [v.value for v in Verb]:
            error_msg = f"Invalid verb '{verb_str}'. Must be: RULE, CONTENT, CARD, GENERATE, ENRICH, FEELGOOD, or PROFILE"
            logger.error(f"Invalid envelope: {error_msg}")
            if self.websocket:
                await self.websocket.send_json(make_error(
                    session_id=session_id,
